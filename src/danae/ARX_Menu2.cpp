...
void ARX_QuickSave()
{
    ...
    for( int iI = 1 ; iI < (save_c) ; iI++ )
    {
        ...
        if( szMenuText.find( tex2 ) != string::npos )
        {
            bFound0     = true;
            sTime0      = save_l[iI].stime;
            iNbSave0    = iI;
        }
        else if( szMenuText1.find( tex2 ) != string::npos )
        {
            bFound1     = true;
    ...
    char    tcSrc[256] = "SCT_0.BMP";
    char    tcDst[256] = "SCT_1.BMP";
...
//            todo: cast
            GetTextExtentPoint( hDC, _lpszUText.c_str(), _lpszUText.length(), &sSize );
            _iWidth = sSize.cx;
            _iHeight = sSize.cy;
...
#define MACRO_MENU_PRINCIPALE(MACRO_button,MACRO_menu,MACRO_locate,MACRO_check){\
        PAK_UNICODE_GetPrivateProfileString( MACRO_locate, "", szMenuText, 256);\
...
CMenuElementText::CMenuElementText(int _iID, HFONT _pHFont, const std::string& _pText,float _fPosX,float _fPosY,long _lColor,float _fSize,MENUSTATE _eMs) : CMenuElement(_eMs)
{
    ...
    GetTextSize(pHFont, _pText, rZone.right, rZone.bottom);
...
std::string CDirectInput::GetFullNameTouch(int _iVirtualKey)
{
    ...
    case DIK_BUTTON9:
        ...
        break;
    case DIK_BUTTON10:
        PAK_UNICODE_GetPrivateProfileString("system_menus_options_input_customize_controls_button9", _"b10", pText, 256);
        break;
    case DIK_BUTTON11:
